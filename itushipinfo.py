...
def get_vessel_details(url):
	...
	try:
		...
		details['type']=" / ".join(map(lambda x: classifications[x] if x in classifications else x, filter(lambda x: len(x) == 2 or len(x)==3, ship_class.split(' '))))
	except IndexError: pass	
	try:
		details['persons'] = re.findall('Person Capacity(?:\<[^\>]*\>[:\s]*)*([^\<]*)', r.text)[0]
	except IndexError: pass


	return details

def identify_vessel(ais):
